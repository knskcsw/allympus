...
- 修正が完了したら、必ず、起動後の開発サーバーのリンクをわかりやすく伝えてな〜。

## 片付け
- ユーザーからOKもろたら、`main` にマージして `main` を push、ワークツリーとブランチ削除して、起動したサーバ止めてな〜。最後まで頼むで〜。
- `main` マージ後は本体DBに対して `prisma migrate dev`（または `prisma db push`）を実行して最新化してな〜。これも忘れたらあかんで〜。
- 今回の開発作業の中でAGENTS.mdの記述で修正するべきところがあったと思ったら、開発作業のコミットを済ませてからAGENTS.mdを修正して、その後にAGENTS.mdをコミットしてな〜。
